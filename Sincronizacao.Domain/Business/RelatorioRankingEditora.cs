...
{
    public class RelatorioRankingEditora
    {
        public List<RowRelatorioRankingEditora> PesquisarSonorizacoes(int unidadeID, DateTime dataIncial, DateTime dataFinal, int tipoTrilha, int generoID, List<Programa> itemsSelecionados)
        {
            var uniID = unidadeID;
        ...
        public List<RowRelatorioRankingEditora> PesquisarSonorizacoesAnalitico(int unidadeID, DateTime dataIncial, DateTime dataFinal, int tipoTrilha, int generoID, List<Programa> itemsSelecionados)
        {
            var uniID = unidadeID;
            var items = itemsSelecionados;
            List<Programa> programas = new List<Programa>();
            ...
            using (Context repositorio = new Context())
            {
                ...
                var query = repositorio.Autorizacoes
                                      ...
                                      .Include("Musica")
                                      .Include("Sonorizacao")
                                      .Include("s.Sonorizacao.Sincronizacao")
                                      .Include("s.Sonorizacao.Sincronizacao.Exibicao")
                                      .AsNoTracking()
                                      .Where(
                                           aut =>
                                           EntityFunctions.DiffDays(dataIncial, aut.Vencimento).Value <= diff.TotalDays &&
                                           EntityFunctions.DiffDays(dataIncial, aut.Vencimento).Value >= 0)
                ...
                var resultSons = sons.OrderBy(s => s.Sonorizacao.Sincronizacao.Exibicao.Data)
                                     .ThenBy(s => s.Sonorizacao.Sincronizacao.Exibicao.Programa.Nome)
                                     .Select(s =>
                                             new RowRelatorioRankingEditora()
                                             {
                                                 Autorizacao = s
                                             });

                var groupSons = from s in resultSons
                                group s by new { s.Autorizacao}
                                    into sg
                                    select sg;

                var groupList = groupSons.ToList().OrderBy(s => s.FirstOrDefault().Editora.Nome)
                                     .ThenBy(s => s.FirstOrDefault().Autorizacao.Musica.Titulo);
